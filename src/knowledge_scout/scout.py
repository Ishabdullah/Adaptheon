...
from .verifiers.composite_verifier import CompositeVerifier
...
class KnowledgeScout:
    ...
    def scout(self, question, category='general'):
        print(f"
  [Knowledge Scout] Launching for: '{question}'")
        ...
        print(f"  [Verification] Confidence: {verification.confidence:.2f} | Passed: {verification.passed}")
        ...
        if should_store:
            ...
            knowledge_key = f"knowledge_{question.lower().replace(' ', '_')[:50]}"
            ...
            print(f"  [Scout] ✓ Knowledge stored in semantic memory")
        else:
            print(f"  [Scout] ✗ Confidence too low, not storing")
        ...
        return {
            ...
            'time_ms': total_time,
            'citations': [{'title': c.title, 'url': c.url} for c in fetch_result.citations]
        }
